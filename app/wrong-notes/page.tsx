    ...
    <main className="mx-auto min-h-screen w-full max-w-5xl px-4 py-8 sm:px-6">
      ...
      <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-2 text-sm sm:grid-cols-3">
          ...
          <select value={showMastered ? "show" : "hide"} onChange={(e) => setShowMastered(e.target.value === "show")} className="rounded-lg border border-slate-300 px-3 py-2">
            <option value="show">숙달 문제 보기</option>
            <option value="hide">숙달 문제 숨기기</option>
          </select>
          <select value={masteredBottom ? "bottom" : "mixed"} onChange={(e) => setMasteredBottom(e.target.value === "bottom")} className="rounded-lg border border-slate-300 px-3 py-2">
            <option value="bottom">숙달 문제 아래로 보내기</option>
            <option value="mixed">숙달 문제 섞어서 보기</option>
          </select>
        </div>
